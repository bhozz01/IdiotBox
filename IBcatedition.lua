...
local options = {
   ...
   ["Miscellaneous"] = {
	   ...
	   }, 
	   {
		   ...
		   {"Spectators Window", "Checkbox", false, 78}, -- Disabled by default
		   {"Radar Window", "Checkbox", false, 78}, -- Disabled by default
		   {"Radar Names", "Checkbox", false, 78}, -- Disabled by default
		   {"Radar Distance:", "Slider", 50, 100, 156}, 
		   {""}, 
		   {"Debug Info", "Checkbox", false, 78}, -- Disabled by default
		   {"Players List", "Checkbox", false, 78}, -- Disabled by default
	...
	if system.IsWindows() and ((BRANCH == "x86-64" and not file.Exists("lua/bin/gmcl_zxcmodule_win64.dll", "MOD")) or (BRANCH == "unknown" and (not file.Exists("lua/bin/gmcl_big_win32.dll", "MOD") or not file.Exists("lua/bin/gmcl_chatclear_win32.dll", "MOD")))) then
		...
		if BRANCH == "unknown" then
			...
			if not file.Exists("lua/bin/gmcl_chatclear_win32.dll", "MOD") then
				chat.AddText(Color(255, 255, 255), "- gmcl_chatclear_win32.dll (Clear chat module)")
			end
		end
		surface.PlaySound("buttons/lightswitch2.wav")
	end
	...
	if bMouse and input.IsMouseDown(MOUSE_LEFT) and not drawlast and not mousedown or notyetselected == check then
		...
		elseif text == "Apply Custom Name" then
			if usedmodules["zxcmodule"] then
				...
				timer.Create("PlaySound", 0.1, 1, function() surface.PlaySound("buttons/lightswitch2.wav") end)
			elseif usedmodules["big"] then
				big.ChangeName(string.gsub(GetConVarString("ib_changename"), "\\n", "\n"))
				timer.Create("ChatPrint", 0.1, 1, function() Popup(3.2, "Successfully applied custom username!", Color(0, 255, 0)) end)
				timer.Create("PlaySound", 0.1, 1, function() surface.PlaySound("buttons/lightswitch2.wav") end)
			else
				timer.Create("ChatPrint", 0.1, 1, function() Popup(3.2, "You need a main module for this feature!", Color(255, 0, 0)) end)
				timer.Create("PlaySound", 0.1, 1, function() surface.PlaySound("buttons/lightswitch2.wav") end)
			end
		elseif text == "Custom Disconnect" then
			if usedmodules["zxcmodule"] then
				...
				timer.Create("ChatPrint", 0.1, 1, function() Popup(3.2, "You need the zxc module for this feature!", Color(255, 0, 0)) end)
				timer.Create("PlaySound", 0.1, 1, function() surface.PlaySound("buttons/lightswitch2.wav") end)
			end
...
local function ChatSpam()
	...
	local spamCategories = {
		["Clear Chat"] = {function()
			if usedmodules["chatclear"] then
				ChatClear.Run()
			else
				timer.Create("ChatPrint", 0.1, 1, function() Popup(3.2, "You need the ChatClear module for this feature!", Color(255, 0, 0)) end)
				timer.Create("PlaySound", 0.1, 1, function() surface.PlaySound("buttons/lightswitch2.wav") end)
			end
		end},
		["OOC Clear Chat"] = {function()
			if usedmodules["chatclear"] then
				ChatClear.OOC()
			else
				timer.Create("ChatPrint", 0.1, 1, function() Popup(3.2, "You need the ChatClear module for this feature!", Color(255, 0, 0)) end)
...
hook.Add("CreateMove", "CreateMove", function(cmd)
   ...
   ib.AirStuck(cmd)
   ib.LaserBullets(cmd)
   if usedmodules["zxcmodule"] then ded.StartPrediction(cmd) end
   if usedmodules["big"] then big.StartPrediction(cmd, cm.CommandNumber(cmd)) end
   Aimbot(cmd)
   Triggerbot(cmd)
   if usedmodules["zxcmodule"] then ded.FinishPrediction() end
   if usedmodules["big"] then big.FinishPrediction() end
end)
