...
ib.contributors["STEAM_0:1:565888199"] = {} -- software (code dev)
...
local function DrawText(w, h, title)
    ...
    if title == "IdiotBox v7.4.b1" then
        surface.SetTextPos(147, 18 - th / 2)
        surface.SetFont("MainFont2")
        surface.DrawText("Latest build: d23m11-pre01")
    end
end
...
local function DrawToggle(self, w, h, var, maxy, posx, posy, dist) -- Thank you ChatGPT for patching up this absolute fucking mess
    local text = var[1]
    local size = var[4]
    surface.SetFont("MenuFont")
    surface.SetTextColor(menutextcol.r, menutextcol.g, menutextcol.b, gInt("Adjustments", "Others", "Text Opacity:"))
    ...
    for i = 1, 159 do
        if input.IsKeyDown(i) or input.IsMouseDown(i) then
            if i == KEY_LWIN or i == KEY_RWIN or  i == KEY_CAPSLOCK or i == KEY_CAPSLOCKTOGGLE or i == KEY_SCROLLLOCK or i == KEY_SCROLLLOCKTOGGLE or i == KEY_NUMLOCK or i == KEY_NUMLOCKTOGGLE then
                continue
            elseif i == KEY_BACKSPACE or i == KEY_ESCAPE or i == KEY_INSERT or i == KEY_HOME or i == KEY_F11 then
                var[3] = 0
                var[5] = 0
                KeyPressed = true
                break
	...
	if var[3] then
		...
		if var[5] == 0 then
			if var[3] == 0 then
				...
				local keyName = input.GetKeyName(var[3])
				if keyName then
...
function ib.Changelog() -- Ran out of local variables, again
	print("===============================================================================================\n\n")
	print("IdiotBox v7.4.b1 GENERAL BUGFIXES")
	print("")
	...
	print("- Fixed Auto Fire bug where it would stop working in some instances, completely at random (known 64 bit issue);")
	...
	print("\n")
	print("IdiotBox v7.4.b1 ADDITIONS & CHANGES")
	print("")
	...
	print("- Reworked 'Fake Lag' from scratch;")
...
local function DrawButton(self, w, h, var, maxy, posx, posy, dist)
	...
	if bMouse or notyetselected == check then
		...
		if feat == "Entity Finder Menu" then
			info = "[ BUTTON NOT WORKING YET!! ]   Allows you to highlight any entitiy. Enable by toggling 'Show Entities' from Visuals > Miscellaneous."
		elseif feat == "Plugin Loader Menu" then
			info = "[ BUTTON NOT WORKING YET!! ]   Allows you to load any Lua script located in your Garry's Mod 'lua' folder."
	...
	if bMouse and input.IsMouseDown(MOUSE_LEFT) and not drawlast and not mousedown or notyetselected == check then
		...
		elseif text == "Custom Disconnect" then
			idiotbox.NetDisconnect(GetConVarString("ib_customdisconnect"))
...
local function Tick()
	...
	if gBool("Aim Assist", "Miscellaneous", "Disable Interpolation") then
		if not applied then
			...
			idiotbox.SetShouldInterpolate(false)
			applied = true
		end
	else
		if applied then
			me:ConCommand("cl_interp 0; cl_interp_ratio 2; cl_updaterate 30")
			idiotbox.SetShouldInterpolate(true)
			applied = false
		end
	end
...
hook.Add("Tick", "Tick", function()
	...
	if (input.IsKeyDown(KEY_INSERT) or input.IsKeyDown(KEY_F11) or input.IsKeyDown(KEY_HOME)) and not menukeydown then
		...
		Menu()
		EntityFinder()
		PluginLoader() -- Make a bool for these two FUCKING sub-menus, outside of the DrawButton context
    end
...
local function OnScreen(v)
	local x, y = idiotbox.WorldToScreen(v:LocalToWorld(v:OBBCenter()))
	if math.abs(x) < ScrW() * 5 and math.abs(y) < ScrH() * 5 then
		return true
	else
		return false
	end
...
function ib.MultiTap(cmd)
	if gBool("Aim Assist", "Miscellaneous", "Multi-Tap") and !FixTools() then
		...
		if attack and not ib.attackLastTick and not wep:IsScripted() then
			idiotbox.SetOutSequenceNr(idiotbox.GetOutSequenceNr() + ib.tickRate - 1)
...
local function AutoFire(cmd)
	if cm.KeyDown(cmd, 1) then
		...
		cm.AddKey(cmd, IN_ATTACK)
	end
end

local function AutoZoom(cmd)
	if cm.KeyDown(cmd, 1) then
		cm.RemoveKey(cmd, IN_ATTACK2)
	else
		cm.AddKey(cmd, IN_ATTACK2)
	end
end
...
hook.Add("PreRender", "PreRender", function() -- S0lum code B)))
        ...
        if me:Alive() then
            aaply2:SetModel(me:GetModel())
            aaply2:SetPos(me:GetPos())
            aaply2:SetAngles(Angle(0,realAngles.oy,0))
            aaply2:SetPoseParameter("aim_pitch",realAngles.ox)
            aaply2:SetPoseParameter("move_x",me:GetPoseParameter("move_x"))
            aaply2:SetPoseParameter("move_y",me:GetPoseParameter("move_y"))
            aaply2:SetPoseParameter("head_pitch",realAngles.ox)
            aaply2:SetPoseParameter("body_yaw",realAngles.oy)
            aaply2:SetPoseParameter("aim_yaw",0)
...
local function FakeLag(cmd)
    ...
    if (not gBool("Hack vs. Hack", "Fake Lag", "Enabled")) or (gBool("Hack vs. Hack", "Fake Lag", "Disable on Attack") and me:KeyDown(IN_ATTACK)) then
        global.bSendPacket = true
    else
...
function ib.PingSpoofer(cmd)
	if gBool("Main Menu", "Miscellaneous", "Ping Spoofer") then
		idiotbox.SetInSequenceNr(idiotbox.GetInSequenceNr() +- gInt("Main Menu", "Miscellaneous", "Spoof Multiplier:"))
	end
end
...
timer.Simple(0.1, function()
	chat.AddText(Color(255, 255, 0), "Attention! You are currently using the developer version.")
	chat.AddText(Color(255, 255, 0), "Some features may be unstable, due to their work-in-progress nature.")
	surface.PlaySound("buttons/lightswitch2.wav")
end)
